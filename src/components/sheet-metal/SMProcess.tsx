...
const DEFAULTS = {
  heading: "Our Sheet Metal ",
  headingHighlight: "Process",
  subheading:
    "Our comprehensive sheet metal fabrication workflow ensures precision, quality, and consistency from design to final part delivery.",
  ...
  steps: [
    {
      ...
      title: "Design & DFM",
      description:
        "3D model analysis and Design for Manufacturability feedback to optimize part design for sheet metal.",
    ...
    },
    {
      ...
      title: "Laser Cutting",
      description:
        "Precision fiber laser cutting of flat sheet stock to produce blanks and profiles.",
    ...
    },
    {
      ...
      description:
        "CNC press brake bending and forming operations to create 3D geometry from flat blanks.",
    ...
    },
    {
      ...
      description:
        "TIG/MIG welding, hardware insertion, and mechanical assembly to complete the part.",
    ...
    },
    {
      ...
      title: "Finishing & QC",
      description:
        "Surface finishing, dimensional inspection, and packaging for shipment.",
  ...
  return (
    ...
    >
      <div className="max-w-[1200px] mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center" style={{ marginBottom: "64px" }}>
          <h2 className="text-white" style={{ fontSize: "46px", fontWeight: 700, letterSpacing: "-0.015em" }}>
            ...
            <span style={{ color: "#EEC569" }}><EditableText path="process.headingHighlight" defaultValue={DEFAULTS.headingHighlight} /></span>
          </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5" style={{ gap: "24px" }}>
          {DEFAULTS.steps.map((step, index) => (
            <motion.div key={step.number} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="relative text-center transition-all duration-300 hover:-translate-y-1">
              <div className="mx-auto flex items-center justify-center" style={{ width: "64px", height: "64px", borderRadius: "50%", border: "2px solid rgba(208,153,71,0.3)", background: "rgba(255,255,255,0.03)", marginBottom: "20px" }}>
                <svg width="28" height="28" viewBox="0 0 24 24" fill="#D09947"><path d={step.iconPath} /></svg>
              </div>
              ...
              <p style={{ fontSize: "13px", lineHeight: 1.6, color: "#C5C6C9" }}>
                <EditableText path={`process.steps.${index}.description`} defaultValue={step.description} multiline />
              </p>
              {index < DEFAULTS.steps.length - 1 && (
                <div className="hidden lg:block absolute top-8 -right-3 w-6 h-[2px]" style={{ background: "linear-gradient(to right, #D09947, transparent)" }} />
              )}
            </motion.div>
          ))}
        </div>
